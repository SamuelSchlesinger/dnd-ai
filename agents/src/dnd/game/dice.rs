...
impl DiceExpression {
    ...
    fn parse_component(
        ...
        if let Some(d_pos) = s.find('d') {
            ...
            // Check for keep highest/lowest notation (e.g., 4d6kh3, 2d20kl1)
...
/// Convenience struct for quick rolls
#[derive(Debug, Clone)]
pub struct DiceRoll {
    pub notation: String,
    pub purpose: String,
...
impl DiceRoll {
    pub fn new(notation: impl Into<String>, purpose: impl Into<String>) -> Self {
        Self {
    ...
    pub fn roll(&self) -> Result<RollResult, DiceError> {
        roll_with_advantage(&self.notation, self.advantage)
    }
}
