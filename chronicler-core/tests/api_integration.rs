...
use chronicler_core::dm::story_memory::ConsequenceSeverity;
use chronicler_core::dm::{DmConfig, DungeonMaster, RelevanceChecker, StoryMemory};
...
fn create_sample_wizard(name: &str) -> Character {
    ...
    character.race = Race {
        ...
        subrace: None,
        race_type: Some(chronicler_core::world::RaceType::Human),
    };
    character.classes = vec![chronicler_core::world::ClassLevel {
...
fn add_hostile_npc(world: &mut GameWorld, name: &str, description: &str) {
    ...
    npc.disposition = chronicler_core::world::Disposition::Hostile;
...
#[tokio::test]
#[ignore] // Run with: cargo test -p chronicler-core --test api_integration -- --ignored
...
async fn test_relevance_checker_with_real_api() {
    ...
    let riverside_id = story_memory.create_entity(
        chronicler_core::dm::EntityType::Location,
        "Riverside Village",
    );

    // Add a consequence about entering the village
    let consequence = chronicler_core::dm::Consequence::new(
...
async fn test_dm_handles_attack_roll() {
    ...
    let has_attack_intent = response
        .intents
        .iter()
        .any(|i| matches!(i, chronicler_core::rules::Intent::Attack { .. }));
    ...
    let has_damage_effect = response.effects.iter().any(
        |e| matches!(e, chronicler_core::rules::Effect::HpChanged { amount, .. } if *amount < 0),
    );
...
async fn test_dm_handles_skill_check() {
    ...
    let has_skill_check = response.intents.iter().any(|i| {
        matches!(
            i,
            chronicler_core::rules::Intent::SkillCheck { .. }
                | chronicler_core::rules::Intent::AbilityCheck { .. }
        )
    });
...
async fn test_dm_handles_short_rest() {
    ...
    let has_rest_intent = response
        .intents
        .iter()
        .any(|i| matches!(i, chronicler_core::rules::Intent::ShortRest));
...
async fn test_dm_handles_long_rest_spell_recovery() {
    ...
    let has_rest_intent = response
        .intents
        .iter()
        .any(|i| matches!(i, chronicler_core::rules::Intent::LongRest));
